# <picture><source media="(prefers-color-scheme: dark)" srcset="docs/references/badges/openchamber-logo-dark.svg"><img src="docs/references/badges/openchamber-logo-light.svg" width="32" height="32" align="absmiddle" /></picture> OpenChamber
  1. Prerequisites
  Ensure you have the OpenCode CLI (https://opencode.ai) installed, as OpenChamber acts as a rich interface for
  it.
  ```
  ...
  3. Running Development Scripts
  Depending on what part of the project you want to work on, you have several options:

   * Standard Web Dev (Full Stack):
   1     bun run dev
      This starts the backend server, the frontend build watcher, and the UI type-checker concurrently. This is
  the most common way to run the project.

   * Web Dev with HMR (Recommended for UI work):
  ...
  This provides Hot Module Replacement (HMR) via Vite. Open the Vite URL (usually http://localhost:5180) for
